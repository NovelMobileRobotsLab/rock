...
class RockEnv:
    ...
    env_cfg = {
        ...
        "num_obs_per_step": 15, #without linvel
    ...
    def __init__(self, num_envs:int, env_cfg=None, show_viewer=False, add_camera=False, viewer_timescale=0.5, device="cuda"):
        ...
        self.scene.build(n_envs=num_envs, env_spacing=(0.5, 0.5))
        ...
        # kvarr = self.cfg["kv_range"][0] + (self.cfg["kv_range"][1] - self.cfg["kv_range"][0]) * torch.rand(self.scene.n_envs, len(self.motor_dofs)).flatten()
        # print(f"kvarr before: {kvarr}")
        self.get_robot().set_dofs_kv(
            kv = self.env_cfg['kv'],
            dofs_idx_local=self.motor_dofs,
        )
        print("kv after: ", self.get_robot().get_dofs_kv())
    ...
    def quatmult(self, q1, q2):
        # Assuming q1 and q2 are torch tensors of shape (n, 4)
    ...
    def step(self, actions):
        ...
        self.get_robot().control_dofs_velocity(motor_speed_des, self.motor_dofs)
